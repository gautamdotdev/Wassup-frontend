...
export function AttachmentPanel({
  ...
  onClose: () => void;
  /** Now passes { url: base64 preview, file: File } items */
  onAddImages: (items: { url: string; file: File }[]) => void;
}) {
  ...
  const handleDevicePick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Promise.all(
      files.map(
        file => new Promise<{ url: string; file: File }>(resolve => {
          const r = new FileReader();
          r.onload = () => resolve({ url: r.result as string, file });
          r.readAsDataURL(file);
        })
      )
    ).then(items => { onAddImages(items); onClose(); });
    e.target.value = "";
  };
